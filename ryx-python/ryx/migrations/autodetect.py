...
class AlterField:
    ...
    def to_python(self) -> str:
        lines = [
            f"    AlterField(",
            f"        table={self.table!r},",
            f"        new_col={_column_state_repr(self.new_col)},",
        ]
        if self.old_col:
...
class Autodetector:
    ...
    def write_migration(self, operations: List[Any]) -> Path:
        ...
        # Collect unique model imports from operations
        model_imports = self._collect_model_imports(operations)
...
'''
    ...
    @staticmethod
    def _collect_model_imports(operations: list) -> list:
        """Build ``from myapp.models import Author`` lines from operation models."""
        imports: Dict[str, list] = {}
        for op in operations:
            model = getattr(op, "model", None)
            if not model:
                continue
            imports.setdefault(model.__module__, []).append(model.__qualname__)
        return [
            f"from {mod} import {', '.join(sorted(set(cls)))}"
            for mod, cls in sorted(imports.items())
        ]
    ...
    def _changes_to_operations(
        ...
        for m in self._models:
            if hasattr(m, "_meta"):
                table_to_model[m._meta.table_name] = m

        ops: List[Any] = []

        for change in changes:
            cls = table_to_model.get(change.table)
        ...
        for model in self._models:
            ...
            for i, fields in enumerate(meta.index_together):
                ...
                ops.append(CreateIndex(table=table, name=name, fields=list(fields), model=model))

            for i, fields in enumerate(meta.unique_together):
                name = f"uq_{table}_{'_'.join(fields)}_{i}"
                ops.append(CreateIndex(table=table, name=name, fields=list(fields), unique=True, model=model))
